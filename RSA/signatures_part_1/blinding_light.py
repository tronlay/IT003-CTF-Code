...
if __name__ == "__main__":
    # 1. Thay port bằng port của bài CTF bạn đang làm
    HOST = 'socket.cryptohack.org'
    PORT = 13376 # Thay số này!
    ADMIN_TOKEN = "admin=True"
    ...
    msg = (bytes_to_long(ADMIN_TOKEN.encode('utf-8')) * pow(2, e, N)) % N

    payload = {  # Sign the ADMIN TOKEN
        'option': 'sign',
        'msg': long_to_bytes(msg).hex()
    }

    result = conn.send_payload(payload)
    signature = (int(result["signature"], 16) * pow(2, -1, N)) % N


    payload = {  # Verify the signature
        'option': 'verify',
        'msg': ADMIN_TOKEN.encode().hex(),
